...
# Workflow SDK language reference
...
- **Workflow builder:** `.add()`, `.to()`, `.group()`
- **Node builder:** `.input()`, `.output()`, `.onError()`
- **Control flow:** `.onTrue()`, `.onFalse()`, `.onCase()`, `.onEachBatch()`, `.onDone()`
- **Connection:** `.connect()`

The only non-builder methods available are `JSON.stringify` and the string methods `.repeat()`, `.trim()`. Native array/string methods such as `.join()`, `.map()`, `.filter()`, `.reduce()`, and `.split()` are NOT available.

## Forbidden constructs

- Arrow functions are not allowed. Use fromAi() directly instead of ($) => $.fromAi()
- Function expressions are not allowed in SDK code
- Function declarations are not allowed in SDK code
- Class declarations are not allowed in SDK code
- Class expressions are not allowed in SDK code
- For loops are not allowed in SDK code
- For-in loops are not allowed in SDK code
- For-of loops are not allowed in SDK code
- While loops are not allowed in SDK code
- Do-while loops are not allowed in SDK code
- Try-catch is not allowed in SDK code
- Throw statements are not allowed in SDK code
- With statements are not allowed in SDK code
- Update expressions (++, --) are not allowed in SDK code
- new expressions are not allowed. Use SDK factory functions instead.
- Import declarations are not allowed in SDK code
- Dynamic imports are not allowed in SDK code
- Named exports are not allowed. Use export default only.
- Re-exports are not allowed in SDK code
- Await expressions are not allowed in SDK code
- Yield expressions are not allowed in SDK code
...
- `Reflect`
- `Proxy`
- `Object`: build object literals directly, or transform in a Code node
- `Array`: build array literals directly, or transform in a Code node
- `String`
- `Number`: use numeric literals, or convert in a Code node / expression
- `Boolean`
- `Symbol`
- `BigInt`
- `JSON`: only JSON.stringify is available; parse at runtime in a Code node
- `Math`: compute at runtime in a Code node or an n8n expression
- `Date`: use the $now / $today helpers inside expr()
- `RegExp`: match at runtime in a Code node or an n8n expression
- `Promise`: builder code is synchronous; no async/await or promises
- `WeakRef`
- `WeakMap`
- `WeakSet`
- `Map`: use object literals, or build collections in a Code node
- `Set`: use array literals, or build collections in a Code node
- `ArrayBuffer`
- `SharedArrayBuffer`
- `DataView`
- `Atomics`
